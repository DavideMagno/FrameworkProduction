...
CalculateHedge <- function(xts.training.set, 
  ...
  w <- sparseIndexTracking::spIndexTrack(xts.training.set$universe,
...
GraphWeights <- function(weights, lookup_table) {
  
  p <- data.frame(Index = names(weights),
                   weights = unname(weights)) |> 
    dplyr::filter(weights != 0) |> 
    dplyr::arrange(Index) |> 
    dplyr::mutate(short_flag = grepl("_short", Index)) |> 
    dplyr::mutate(weights = dplyr::if_else(short_flag, -weights, weights)) |> 
    dplyr::mutate(Index = stringr::str_remove_all(Index, "_short")) |> 
    dplyr::select(-short_flag) |> 
    dplyr::left_join(lookup_table, by = "Index") |> 
    hchart(type = "bar", hcaes(x = `Long Name`, y = weights)) |>
    hc_yAxis(title = list(text = "Weight in replicating portfolio")) |> 
