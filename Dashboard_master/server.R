...
shinyServer(function(input, output, session) {
  ...
  user_base <- readRDS(here::here("user_base.rds"))
  ...
  observe({
    ...
    shinyjs::show(id = "show-sidebar-content")
  })
  
  LAG <- reactive({
    lag <- ifelse(grepl("_new",input$asset_manager) & !is.null(input$time_limit), 
                  as.numeric(input$time_limit), 
                  252)
  ...
  FrameworkIndices <- reactive({
    load(glue::glue("{base_dir}/Data/IndexMapping{input$asset_manager}.RData"))
  ...
  prices <- reactive({
    ...
    dir <- glue::glue("{base_dir}/Data/input{input$asset_manager}")
  ...
  statistics <- reactive({
    req(relative_value())
    if (input$adjust_corridor) {
      relative.trade <- list(type = type_index(),
                             trades = relative_value()$trades_weighted)
    } else {
      relative.trade <- list(type = type_index(),
  ...
  stats.relative.trade <- reactive({
    req(relative_value())
    if (input$adjust_corridor) {
      performance <- tail(relative_value()$performance_weighted, -1)
    } else {
      performance <- tail(relative_value()$performance, -1)
    }
    
    return <- PerformanceAnalytics::Return.annualized(performance[,c(2,1)],
                                                      geometric = type_index())
    
    sd <- PerformanceAnalytics::sd.annualized(performance[,c(2,1)],
                                              scale = 252)
    
    sharpe <- PerformanceAnalytics::SharpeRatio.annualized(performance[,c(2,1)],
                                                           scale = 252, 
                                                           geometric = type_index())
    
    drawdown <- PerformanceAnalytics::maxDrawdown(performance[,c(2,1)],
                                                  geometric = type_index())
    
    var <- PerformanceAnalytics::VaR(performance[,c(2,1)])
  ...
  landing_table <- reactive({
    ...
    dir <- glue::glue("{base_dir}/Data/output_distance{input$asset_manager}")
    
    market_data <- glue::glue("{dir}/K={K()}") |> 
      arrow::open_dataset() |>
      dplyr::collect() |> 
      dplyr::filter(Dates %in% end_date) |> 
      dplyr::select(-YearMonth, -Dates) 
    
    table <- FrameworkIndices() |> 
      dplyr::mutate(Index = as.character(Index)) |> 
      dplyr::select(Index,`Long Name`, `Asset Class`) |> 
  ...
  lending_table_dt <- reactive({
    ...
    table <- DT::datatable(landing_table(), fillContainer = TRUE, 
                           ...
                           options = list(dom = "t",
                                          ...
                                          paging = FALSE)) |> 
      DT::formatRound("Current Index Value", 3) |> 
      DT::formatRound("Distance from Estimated Fair Value (z-score)", 3) 
    
    if (grepl("_new",input$asset_manager)) {
      table <- table |>
        DT::formatRound("Information Index", 3) |>
        DT::formatRound("Mean Information Index", 3)  
    }
  ...
  output$page_report <- downloadHandler(
    ...
    content <- function(file) {
      ...
      if (input$adjust_corridor) {
        range <- relative_value()$range_weighted
        trades <- relative_value()$trades_weighted
        trades.daily <- relative_value()$replica.daily_weighted
      } else {
  ...
  observeEvent(input$final_report,{
    ...
    start_date <- input$dates_prices[1]
    end_date <- input$dates_prices[2]
    dir <- glue::glue("{base_dir}/Data/output{input$asset_manager}")
    ...
    for (i in looping.array) {
      ...
      inputs <- GetDataFromParquet(K(), LAG(), index, start_date, end_date,
                                   dir, flag_weighted = TRUE)
      inputs <- c(inputs, list(type = type()[i]))
      
      result <- AnalysisForRecommendation(inputs, index, input$adjust_corridor)
  ...
  relative_value <- reactive({
    ...
    data <- GetDataFromParquet(K(), LAG, index, start_date, end_date, dir,
                               flag_weighted = TRUE)
  ...
  output$control_centre_relative_value <- renderUI({
    if (grepl("_new",input$asset_manager)) {
      fluidRow(
        column(3,align = "center", uiOutput('relative.value.asset')),
        ...
        column(3, align = "center", pickerInput(inputId = "time_limit",
                                                label = "Select the Trades' Time Limit",
                                                selected = "None",
                                                choices = time_limit_choices,
                                                multiple = FALSE)),
        column(2, align = "center", switchInput(
          inputId = "adjust_corridor",
          label = "Adjust the corridor by the Information Ratio", 
          labelWidth = "250px",
          width = "250px"
        )),
        column(2,align = "center", downloadButton("page_report", "Download the trading data",
                                                  style = "margin-top:28;"))
      )
    } else {
      fluidRow(
  ...
  output$date.selection <- renderUI({
    ...
    prices <- arrow::open_dataset(glue::glue("{base_dir}/Data/input{input$asset_manager}")) |> 
      dplyr::collect()
  ...
  output$index_and_eigendata <- highcharter::renderHighchart({
    ...
    if (grepl("_new",input$asset_manager)) {
      dir_info <- glue::glue("{base_dir}/Data/output_distance{input$asset_manager}/K={K()}")
      graph_index_and_eigendata(K(), LAG(), index, start_date, end_date, 
                                dir = dir,
  ...
  output$graph_distance_TRUE <- highcharter::renderHighchart({
    req(relative_value())
    chosen_index <- FrameworkIndices()$`Long Name`[FrameworkIndices()$Index %in% input$index.strategy]
    
    start_date <- input$dates_prices[1]
    end_date <- input$dates_prices[2]
    
    if (input$adjust_corridor) {
      trades.graph <- relative_value()$trades_graph_weighted
      trades <- relative_value()$trades_weighted
      range <- relative_value()$range_weighted
      range.graph <- relative_value()$range_graph_weighted
      other.range <- relative_value()$range
      other.range.graph <- relative_value()$range_graph
    } else {
      trades.graph <- relative_value()$trades_graph
      trades <- relative_value()$trades
      range <- relative_value()$range
      range.graph <- relative_value()$range_graph
      other.range <- relative_value()$range_weighted
      other.range.graph <- relative_value()$range_graph_weighted
    }
    
    trades.data <- list(trades.graph = trades.graph,
                        trades = trades,
                        range = list(range.daily = relative_value()$replica.daily,
                                     range = range),
                        range.graph = list(range.daily = relative_value()$replica.daily_graph,
                                           range = range.graph),
                        other.range = list(range.daily = relative_value()$replica.daily,
                                           range = other.range),
                        other.range.graph = list(range.daily = relative_value()$replica.daily,
                                           range = other.range.graph))
    
    delta_t <-end_date - start_date
    
    TradeDistancePlot(trades.data, TRUE, delta_t, input$remove_trades,
                      chosen_index, input$adjust_corridor) 
  })
  ...
  output$graph_pnl_relative <- highcharter::renderHighchart({
    ...
    if (input$adjust_corridor) {
      performance <- relative_value()$performance_weighted
    } else {
      performance <- relative_value()$performance
    }
    
    if (input$main_panel %in% "Relative Value") {
      performance_graphs(performance, geometric = type_index(),
                         input$main_panel)
    }
  ...
  trading_matrix <- reactive({
    source(here::here("Dashboard_master/code_trading.R"))
    calculate_trading_matrix(prices(), input$asset_manager, FrameworkIndices(),
                             K(), LAG())
  })
  
  output$current_trading_portfolio <- renderUI({
    
    if (is.null(trading_matrix()$current_portfolio)) {
      card(full_screen = TRUE,
           card_header("Current Trading Portfolio"),
           fluidRow(
             column(12,
                    span(strong(("The trading portfolio is currently empty")))
             )
           )
      )
    } else {
      data <- trading_matrix()$current_portfolio
      card(full_screen = TRUE,
           card_header("Current Trading Portfolio"),
           fluidRow(
             column(2,  align = "center", span(strong("KEEP IN THE PORTFOLIO?"))),
             column(3,  align = "center", span(strong("INDEX"))),
             column(3,  align = "center", span(strong("TRADE DIRECTION"))),
             column(2,  align = "center", span(strong("NUMBER OF FUTURES"))),
             column(2,  align = "center", span(strong("RECOMMENDATION")))
           ),
           lapply(1:nrow(data), function(i){
             fluidRow(
               column(2, align = "center",
                      style = "margin-bottom: 0px;padding-top: 5px;padding-left: 15px;",
                      awesomeCheckbox(inputId = paste0("keep_",i),
                                      label = "",
                                      value = TRUE)),
               column(3, align = "center", span(strong(data$`Long Name`[i]))),
               column(3, align = "center", span(strong(data$Direction[i]))),
               column(2, align = "center", span(strong(data$`Number of futures`[i]))),
               column(2, align = "center", span(strong(data$signal[i])))
             )
           })
      )
    }
  ...
  output$long_box <- renderUI({
    data <- trading_matrix()$trade_matrix 
    card(full_screen = TRUE,
         card_header("Tradable assets"),
         fluidRow(
           column(3,  align = "center", span(strong("INCLUDE IN THE PORTFOLIO?"))),
           column(3,  align = "center", span(strong("INDEX"))),
           column(3,  align = "center", span(strong("TRADE DIRECTION"))),
           column(3,  align = "center", span(strong("INFORMATION RATIO"))),
         ),
         lapply(1:nrow(data), function(i){
           fluidRow(
             column(3, align = "center", 
                    style = "margin-bottom: 0px;padding-top: 5px;padding-left: 15px;",
                    awesomeCheckbox(inputId = paste0("trade_",i),
                                    label = "",
                                    value = FALSE)
                    # status = "primary",
                    # shape = "curve",
             ...
             column(3, align = "center", span(strong(data$`Long Name`[i]))),
             column(3, align = "center", span(strong(data$signal[i]))),
             column(3, align = "center", span(strong(data$info_ratio[i])))
           )
         })
    )
  })
  
  output$portfolio_rebalanced <- renderUI({
    req(trading_matrix())
    data <- trading_matrix()$trade_matrix
    portfolio <- trading_matrix()$current_portfolio
    card(full_screen = TRUE,
         card_header("Rebalanced Portfolio"),
         fluidRow(
           column(3,  align = "center", span(strong("INDEX"))),
           column(1,  align = "center", span(strong("TRADE DIRECTION"))),
           column(3,  align = "center", span(strong("AVERAGE $ EXCHANGED DAILY"))),
           column(2,  align = "center", span(strong("NUMBER OF FUTURES"))),
           column(3,  align = "center", span(strong("TOTAL $ EXCHANGED DAILY")))
         ),
         if (nrow(portfolio) > 0) {
           lapply(1:nrow(data) , function(i){
             if (input[[paste0("keep_", i)]]) {
               fluidRow(
                 column(3, align = "center", span(strong(portfolio$`Long Name`[i]))),
                 column(1, align = "center", span(strong(portfolio$Direction[i]))),
                 column(3, align = "center", span(strong(portfolio$average_value_exchanged[i]))),
                 column(2, align = "center",
                        isolate(numericInput(inputId = paste0("number_futures_", i), 
                                             label = NULL,
                                             value = portfolio$`Number of futures`[i], min = 0)))#,
                 # column(3, align = "center", span(strong(data$average_value_exchanged[i] * input[[paste0("number_futures_", i)]]))
                 #        )
               )
             }
           })
         ...
         if (nrow(data) > 0) {
           lapply(1:nrow(data) , function(i){
             if (input[[paste0("trade_", i)]]) {
               fluidRow(
                 column(3, align = "center", span(strong(data$`Long Name`[i]))),
                 column(1, align = "center", span(strong(data$signal[i]))),
                 column(3, align = "center", span(strong(data$average_value_exchanged[i]))),
                 column(2, align = "center",
                        isolate(numericInput(inputId = paste0("number_futures_", i), 
                                             label = NULL,
                                             value = 1, min = 0)))#,
  ...
  pairs_trading <- reactive({
    ...
    pairs_trade <- arrow::open_dataset(glue::glue("{base_dir}/Data/\\
