...
PrepareRange <- function(projection, dates, normalise = TRUE, 
  ...
  if(normalise) {
    projection$prices$mean <- rowMeans(projection$prices) 
...
RelativeTradingCone <- function(projection, 
                                dates, lag.window = 0, index.flag = TRUE,
...
SimulateConeParallel <- function(interval, whole.set, whole.set.prices, 
                                 ...
                                 center.flag, lag, rolling.window, once.flag = FALSE,
                                 updateProgress = NULL) {
  ...
  ending.window <- seq(rolling.window + 1, nrow(whole.set.prices), 
                       by = interval) 
  ...
  starting.window <- ending.window - rolling.window
  ...
  training.set <- foreach::foreach(x = head(starting.window, -1),
                                   y = head(ending.window, -1)) %dopar% {
                                     whole.set[x:(y - 1), ]
                                   }
  
  training.set.prices <- foreach::foreach(x = head(starting.window, -1),
                                          y = head(ending.window, -1)) %dopar% {
                                            whole.set.prices[x:y, ]
                                          }
  ...
  ret.index <- purrr::map_dfr(tail(time.series, -1),
                              ~tail(.x$ret.index, interval))
  
  index <- purrr::map_dfr(time.series, ~.x$range)
  
  graph.index <- purrr::map_dfr(tail(time.series, -1),
                                ~tail(.x$index, interval))
  
  ret.index <- dplyr::bind_rows(time.series[[1]]$ret.index, ret.index) 
  graph.index <- dplyr::bind_rows(time.series[[1]]$index, graph.index)
...
CalculateLastDataAtomic <- function(range, range_graph, index, start, 
  ...
  confidence_range <- min(c(confidence_range, 1))
  confidence_range <- max(c(confidence_range, 0))
  
  confidence_range_message <- switch(ceiling(confidence_range*5),
