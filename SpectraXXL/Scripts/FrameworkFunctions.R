source("/home/threshold/Framework/Scripts/InvarianceTest.R")
source("/home/threshold/Framework/Scripts/SVD.R")
source("/home/threshold/Framework/Scripts/RelativeTrade.R")
library(PerformanceAnalytics)

`%not_in%` <- Negate("%in%")

CalculatePrices <- function(LAG, start_date = as.Date("2000-01-04"),
                               end_date = Sys.Date() - lubridate::days(1), dir) {
  ...
  # M <- ifelse(grepl("Max", cut.dates), N - 1, 
  #             as.numeric(cut.dates) * LAG * factor) 
  # 
  # if (LAG == 252 & !grepl("Max", cut.dates)) {
  #   browser()
  #   max.date <- max(prices$dates)
  #   min.date <- max.date - lubridate::years(cut.dates)
  #   
  #   closest.date.position <- which.min(abs(prices$dates - min.date))
  #   
  #   M <- nrow(prices) - closest.date.position
  # }
  # 
  # range.step <- ifelse(LAG == 4, 3, 1)
  # 
  # range <- seq(from = 1, to = N, by = range.step)
  # 
  # start.date <- N - (M + shift.back)
  # prices.monthly.interim <- prices.monthly[start.date:N,]
  # prices.monthly <-  prices.monthly[range,]
  # 
  # N <- nrow(prices.monthly.interim)
  # range <- seq(from = 1, to = N, by = range.step)
  # prices.monthly.cut <-  prices.monthly.interim[range,]
  # 
  # first.date <- min(as.Date(prices.monthly.cut$dates))
  # prices.cut <- prices[prices$dates >= first.date,]
...
Framework <- function(index, prices, returns, type, K, ROLLING.WINDOW, LAG) {
  message(index)
  ...
  returns <- returns$returns.cut |> 
    dplyr::select(dates, index.strategy = dplyr::contains(index)) |> 
    na.omit()
  
  daily.prices <- prices$prices.cut |> 
    dplyr::select(dates, index.strategy = dplyr::contains(index)) |> 
    na.omit()
  ...
  trading$indices <- trading$indices |> 
    dplyr::mutate(Dates = trading$trades$time) |> 
    dplyr::select(Dates, dplyr::everything()) |> 
    dplyr::mutate(YearMonth = as.character(Dates, "%Y-%m"))
...
WriteFullIndexOutput <- function(framework.output, index, K, LAG, 
                                 dir = here::here()) {
  dir <- glue::glue("{dir}/{index}/K={K}/LAG={LAG}")
  if (!dir.exists(dir)) {
  ...
  for (item in items) {
    ...
    if (item %in% "portfolio.return") {
      data_to_store <- as.data.frame(data_to_store) 
      data_to_store$Dates <- rownames(data_to_store)
      rownames(data_to_store) <- NULL
      data_to_store <- data_to_store |> 
        dplyr::select(Dates, dplyr::everything()) |> 
        dplyr::mutate(Dates = as.Date(Dates),
                      YearMonth = as.character(Dates, "%Y-%m"))
